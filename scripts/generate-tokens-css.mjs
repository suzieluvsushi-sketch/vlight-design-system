...
const outputFile = path.join(projectRoot, "src", "tokens.css")
...
const textStyleValue = (style, property, literal) => {
  const binding = style.boundVariables?.[property]

  return binding
    ? `var(${cssVariableName(binding.$target.collection, binding.$target.name)})`
    : literal
}

const textStyleVariables = source.styles.text
  .flatMap((style) => {
    const prefix = `--typography-${slug(style.name)}`

    return [
      `  ${prefix}-font-family: ${textStyleValue(style, "fontFamily", JSON.stringify(style.value.fontFamily))};`,
      `  ${prefix}-font-size: ${textStyleValue(style, "fontSize", `${formatNumber(style.value.fontSize)}px`)};`,
      `  ${prefix}-font-weight: ${textStyleValue(style, "fontWeight", style.value.fontStyle)};`,
      `  ${prefix}-line-height: ${textStyleValue(style, "lineHeight", `${formatNumber(style.value.lineHeight.value)}px`)};`,
    ]
  })
  .join("\n")
...
const css = `/* This file is generated by scripts/generate-tokens-css.mjs. */
