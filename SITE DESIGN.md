...
- **Token ownership/runtime mapping:** `tokens/design-tokens.json` is the canonical source. `npm run tokens:css` deterministically generates `src/tokens.css`, preserving Basic → Semantic → Component references as CSS `var(...)` aliases. `src/base.css` is the optional product baseline; `src/globals.css` imports it and adds documentation-site-only rules. The shadcn variables are an adapter over the Semantic layer, not a second source of truth.
...
|---|---|---:|
| Preview-page main padding | Desktop top `layout/spacing/6`; inline `layout/spacing/10`; bottom `layout/padding/xl` | `24px 40px 32px` |
...
The radius scale is `4 / 8 / 12 / 16 / 20 / 24px`, with an `8px` base radius. Tailwind radius utilities are mapped to these Layout tokens in `src/tokens.css`; documentation-site styles should use those variables or utilities rather than hardcoded corner values.
...
- **Don't:** Edit generated values in `src/tokens.css`; update `tokens/design-tokens.json` or the generator and rebuild the CSS.
